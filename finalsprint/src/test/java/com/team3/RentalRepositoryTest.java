...
public class RentalRepositoryTest {
    ...
    User userUnderTest1 = new User("user1", "password", personUnderTest1);
    Rental rentalUnderTest1 = new Rental(userUnderTest1, "active", "Single", LocalDate.now(), "8", 200.00, "Scenic Hideaway for a weekend getaway", addressUnderTest);
    List<Rental> rentals = List.of(rentalUnderTest1);

    @Test
    public void testFindByRentalManager() {
        
        Mockito.when(rentalRepository.findByRentalManager(userUnderTest1)).thenReturn(rentals);
        List<Rental> rental = rentalRepository.findByRentalManager(userUnderTest1);
